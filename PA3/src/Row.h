...
class Row {
  public:
    ...
    Row(double x = -1, double y = -1, double height = -1, double siteSpacing = -1, unsigned numSites = -1, Orient orient = OR_N,
        bool isSymmetric = true)
        : _x(x), _y(y), _height(height), _siteSpacing(siteSpacing), _numSites(numSites), _orient(orient), _isSymmetric(isSymmetric) {}

    Row(double& set_bottom, double& set_height, double& set_step) : _y(set_bottom), _height(set_height), _siteSpacing(set_step), _orient(OR_N) {}
