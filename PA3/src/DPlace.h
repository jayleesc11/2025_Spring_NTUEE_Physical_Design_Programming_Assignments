...
class Array2D {
  ...
  public:
    ...
    ~Array2D() { delete[] data; }
    void put(int i, int j, T d) { data[i * n + j] = d; }
...
class lap2 {
  public:
    lap2(int deg) {
        m_deg = deg;
        m_cost.resize(m_deg);
        for (unsigned i = 0; i < m_cost.size(); ++i) { m_cost[i].resize(m_deg, 0); }
    ...
    void put(const int& i, const int& j, double wl) { m_cost[i][j] = static_cast<int>(wl); }
    ...
    int lap_solve();
    void getresult(vector<int>& v) { v = this->m_assignment; }
    bool verbose;
};
        ...
        for (unsigned i = 0; i < place.numModules(); i++) {
            ...
            if (curModule.isFixed() == false) {
                ...
                if (curModule.height() > place.getRowHeight()) {
                    ...
                    if (curModule.height() * height < place.getRowHeight()) height++;
                }
                int pos = y2rowID(curModule.y());
                for (unsigned j = 0; j < height; j++) { m_de_row[pos + j].insert_module(curModule.x(), curModule.width(), i); }
            }
        }
    }
    int y2rowID(double y) { return (int)((y - _placement->rectangleChip().bottom()) / _placement->getRowHeight()); }
