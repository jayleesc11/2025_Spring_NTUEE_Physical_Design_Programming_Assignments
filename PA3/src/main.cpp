...
bool handleArgument(const int& argc, char* argv[], CParamPlacement& param) {
    ...
    while (i < argc) {
        ...
        else if (strcmp(argv[i] + 1, "loadpl") == 0) { param.plFilename = string(argv[++i]); }
        i++;
    }
    return true;
}
...
int main(int argc, char* argv[]) {
    ...
    cout << format("Core region: (%.f,%.f)-(%.f,%.f)", placement.boundaryLeft(), placement.boundaryBottom(), placement.boundaryRight(),
                   placement.boundaryTop())
         << endl;

    setConfig(stoi(placement.name().substr(3)));

    // openmp
    omp_set_num_threads(config.kThreads);
    ...
    if (param.bRunGlobal) {
        ...
        globalPlacer.place();
        // globalPlacer.plotPlacementResult("init.plt", true);
