...
class CLegalLocation {
  public:
    CLegalLocation(const int& set_site_index, const double& set_xcoor)
        : m_site_index(set_site_index), m_xcoor(set_xcoor), m_shift(0), m_wirelength(0) {}
...
class LessShift {
  public:
    bool operator()(const CLegalLocation& l1, const CLegalLocation& l2) { return l1.m_shift < l2.m_shift; }
};

// Compare m_wirelength of class 'CLegalLocation'
class LessWirelength {
  public:
    bool operator()(const CLegalLocation& l1, const CLegalLocation& l2) { return l1.m_wirelength < l2.m_wirelength; }
};
...
class LessXCoor {
  public:
    ...
    bool operator()(const CTerminal& t1, const CTerminal& t2) { return t1.m_xcoor < t2.m_xcoor; }
};
...
inline double Rounding(const double& d1) { return floor(d1 + 0.5); }
...
class CTetrisLegal {
  ...
  private:
    ...
    bool AggressiveMacroDiamondSearch(const int& cellid, std::vector<Jin::CLegalLocation>& legalLocations);
    ...
    bool AggressiveCellLegalLocationsSearch(const int& cellid, std::vector<Jin::CLegalLocation>& legalLocations);

    // Return the index of the best location of given locations for a given cell
    int ReturnBestLocationIndex(const int& cellid, std::vector<Jin::CLegalLocation>& legalLocations);

    // Returns the minimum shift between the original cell location and
    // the locations in 'legalLocations'
    double ReturnMinimumShift(const int& cellid, const std::vector<Jin::CLegalLocation>& locations);
    ...
    void GetCellLegalLocationsTowardLeft(const int& cellid, const int start_site_index, const int end_site_index,
                                         std::vector<Jin::CLegalLocation>& legalLocations,  // Put the resulting locations into legalLocations
    ...
    void RestoreOrig(void);                                                      // Restore original solution
    ...
    // Remove occupied free site in m_free_sites[rowId] by the given xstart and width
